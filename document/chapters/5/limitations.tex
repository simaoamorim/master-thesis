...
Instead, a wrapper-launcher can be written which stores the command-line argument values. For example a \verb|bash| script can automatically launch the application with predetermined-determined values.
A single text file called \verb|run.sh| containing the command shown in \autoref{lst:main-sh} has achieved the goal of storing predefined-defined parameter values, so we did not pursue any further options.
...
\lstinputlisting[float=htp,language=bash,label=lst:main-sh,caption=Wrapper script to launch application with predefined-defined parameters]{../src/main/run.sh}
